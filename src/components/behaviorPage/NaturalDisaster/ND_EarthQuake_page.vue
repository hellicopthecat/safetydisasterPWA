...
<template>
  ...
  <v-container class="d-flex flex-column align-center justify-space-around">
    ...
    <v-card max-width="900" class="pa-2 mb-15" :elevation="5">
      ...
      <v-card-text>
        <p v-for="when in whenEarthQuake[1]" :key="when">
          {{ when }}
        </p>
      </v-card-text>
    </v-card>
    ...
    <v-card min-width="900" class="pa-2 mb-15" :elevation="5">
      ...
      <v-card-text>
        <p v-for="preparePlace in preparePlaceEarthQuake[1]" :key="preparePlace">
          {{ preparePlace }}
        </p>
      </v-card-text>
    </v-card>
    ...
    <v-card max-width="900" class="pa-2 mb-15" :elevation="5">
      ...
      <v-card-text>
        <p v-for="escapeEarth in escapeEarthQuake[1]" :key="escapeEarth">
          {{ escapeEarth }}
        </p>
      </v-card-text>
    </v-card>
